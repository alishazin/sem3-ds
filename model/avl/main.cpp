...
class AVL {
    ...
    Node* rightRotate(Node* z) {
        ...
        z->height = 1 + this->max(this->height(z->right), this->height(z->left));
        y->height = 1 + this->max(this->height(y->right), this->height(y->left));
    ...
    Node* _Insert(Node* root, int value) {
        ...
        if (root == NULL) return this->createNode(value);
